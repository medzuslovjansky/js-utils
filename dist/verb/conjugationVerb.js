...
exports.conjugationVerb = exports._getConjugationVerbFlat = exports.conjugationVerbFlat = void 0;
...
function conjugationVerbFlat(inf, rawPts, partOfSpeech) {
    return _getConjugationVerbFlat(conjugationVerb(inf, rawPts, partOfSpeech));
}
exports.conjugationVerbFlat = conjugationVerbFlat;
/** @internal */
function _getConjugationVerbFlat(result) {
    if (!result) {
        return [];
    }
    const forms = [
        ...(0, utils_1.compactArray)(result.conditional).map((item) => item.split(' ')[1].replace(/[()]/g, '')),
        result.gerund,
        ...result.imperative.replace(/ /g, '').split(','),
        ...result.imperfect,
        result.infinitive,
        ...(0, utils_1.compactArray)([
            result.pfap,
            result.pfpp,
            result.prap,
            result.prpp,
...
exports._getConjugationVerbFlat = _getConjugationVerbFlat;
function conjugationVerb(rawInf, rawPts, partOfSpeech = 'v.tr. ipf./pf.') {
    ...
    const { imperfective, transitive } = (0, partOfSpeech_1.parsePos)(partOfSpeech);
    const prap = imperfective ? build_prap(pref, ps, refl) : undefined;
    const prpp = imperfective && transitive ? build_prpp(pref, ps, psi) : undefined;
...
exports.conjugationVerb = conjugationVerb;
function splitReflexive(inf) {
    const hasNE = inf.startsWith('ne ');
    const spaceIndex = inf.indexOf(' ', hasNE ? 3 : 0);
    const maybeSE = spaceIndex > 0 ? inf.slice(spaceIndex + 1, spaceIndex + 3) : '';
...
function derive_present_tense_stem(infinitive_stem_string) {
    ...
    else if (result.endsWith(substitutions_1.IOTATED_SMALL_YUS)) {
        result = result.slice(0, -1) + 'm';
    }
    else if (result.endsWith(substitutions_1.SMALL_YUS)) {
        result = result.slice(0, -1) + 'n';
    }
    else if (result.endsWith(substitutions_1.BIG_YUS)) {
        result = result.slice(0, -1) /*+ 'm'*/;
    }
...
function process_present_tense_stem_exceptions(pref, is, result) {
    if ((is == 'by' && pref == '') ||
...
function buildPresent(pref, ps, psi, refl) {
    ...
    switch (ps) {
        case 'jes':
            return ['jesm', 'jesi', 'jest (je)', 'jesmo', 'jeste', 'sųt'].map(transliterateBack);
        case 'da':
            return ['dam', 'daš', 'da', 'damo', 'date', 'dadųt'].map((word) => transliterateBack(`${pref}${word}${refl}`));
        case 'vě':
            return ['věm', 'věš', 'vě', 'věmo', 'věte', 'vědųt'].map((word) => transliterateBack(`${pref}${word}${refl}`));
        case 'jě':
            return ['jěm', 'jěš', 'jě', 'jěmo', 'jěte', 'jědųt'].map((word) => transliterateBack(`${pref}${word}${refl}`));
        case 'je':
            return ['jem', 'ješ', 'je', 'jemo', 'jete', 'jedųt'].map((word) => transliterateBack(`${pref}${word}${refl}`));
        case 'ja':
            return ['jam', 'jaš', 'ja', 'jamo', 'jate', 'jadųt'].map((word) => transliterateBack(`${pref}${word}${refl}`));
    }
    switch (ps.slice(-1)) {
        case 'ĵ': {
            // Naučny Medžuslovjanski uses...
...
function buildFuture(infinitive, ps) {
    if (((infinitive == 'biti' || infinitive == 'бити') &&
        (ps == 'j' || ps == 'je' || ps == 'jes')) ||
        infinitive == 'byti' ||
        infinitive == 'bytì' ||
        infinitive == 'быти') {
...
function build_pfpp(pref, is, psi) {
    ...
    if ((is.match(/[iyuě]$/) && psi.match(/[jvn]$/) && psi !== 'imaj') ||
        is.match(/[ęuųå]$/) ||
        is === 'by' ||
...
function build_gerund(pfpp) {
    const ppps = pfpp.indexOf('(') - 2;
    return transliterateBack(pfpp.substring(0, ppps) + 'ıje' /*+ refl*/).replace('ne ', 'ne');
}
function idti(sel) {
    return sel
...
//# sourceMappingURL=conjugationVerb.js.map