...
exports.declensionNoun = exports._getDeclensionNounFlat = exports.declensionNounFlat = void 0;
const adjective_1 = require("../adjective");
const common_1 = require("../common");
const utils_1 = require("../utils");
const establishGender_1 = require("./establishGender");
// endings like -i, -u are not declinable usually
const AEEO$ = /[aeęo]$/;
// should omit words like: obUv, žOLv
const XV$ = /(?:[^aåeęěėioȯuųyl]|[^oȯ]l)v$/;
function declensionNounFlat(rawNoun, rawAdd, originGender, animated, isPlural, isSingular, isIndeclinable) {
    return _getDeclensionNounFlat(declensionNoun(rawNoun, rawAdd, originGender, animated, isPlural, isSingular, isIndeclinable));
}
exports.declensionNounFlat = declensionNounFlat;
/** @internal */
function _getDeclensionNounFlat(result) {
    if (!result) {
        return [];
    }
...
exports._getDeclensionNounFlat = _getDeclensionNounFlat;
function declensionNoun(rawNoun, rawAdd, originGender, animated, isPlural, isSingular, isIndeclinable) {
    // remove square brackets
    let noun = (0, utils_1.removeBrackets)(rawNoun, '[', ']');
    ...
    if (add &&
        ['-ogo', '-ego', '-oj', '-ej'].indexOf(add.replace(noun.slice(0, -1), '-')) !== -1) {
    ...
    if (add && noun !== add) {
        noun = (0, common_1.markFleetingVowel)(noun, add);
    }
    else if (originGender === 'masculine') {
        noun = (0, common_1.inferFleetingVowel)(noun);
    }
    ...
    const gender = (0, establishGender_1.establishGender)(noun, rawGender);
...
exports.declensionNoun = declensionNoun;
...
function establish_root(noun, gender) {
    ...
    const fleetingVowelIndex = Math.max(noun.lastIndexOf('ė'), noun.lastIndexOf('ȯ'));
    ...
    if (!hasVowelEnding && fleetingVowelIndex > result.length - 3) {
        result = (0, utils_1.replaceStringAt)(result, fleetingVowelIndex, '');
    }
    return result;
}
...
function declensionSubstAdj(word, add, gender, animated) {
    if (gender === 'masculine' || gender === 'neuter') {
        const adjectiveParadigm = (0, adjective_1.declensionAdjective)(word.slice(0, -1) + (add === '-ogo' ? 'y' : 'i'), '');
        ...
        const adjectiveParadigm = (0, adjective_1.declensionAdjective)(word.slice(0, -1) + (add === '-oj' ? 'y' : 'i'), '');
        return {
...
//# sourceMappingURL=declensionNoun.js.map