...
exports.declensionAdjective = exports._getDeclensionAdjectiveFlat = exports.declensionAdjectiveFlat = void 0;
const substitutions_1 = require("../substitutions");
function declensionAdjectiveFlat(adj, postfix, partOfSpeech) {
    return _getDeclensionAdjectiveFlat(declensionAdjective(adj, postfix, partOfSpeech));
}
exports.declensionAdjectiveFlat = declensionAdjectiveFlat;
/** @internal */
function _getDeclensionAdjectiveFlat(result) {
    const forms = [];
    for (const key of Object.keys(result)) {
        ...
        if (typeof value === 'string') {
            ...
            const notFlat = Object.values(value);
...
exports._getDeclensionAdjectiveFlat = _getDeclensionAdjectiveFlat;
const LOOKS_LIKE_ADJECTIVE = /[yij]$/;
function looksLikeAdjective(word) {
    return LOOKS_LIKE_ADJECTIVE.test(word);
}
function declensionAdjective(adj, postfix = '', partOfSpeech = 'adj.') {
...
exports.declensionAdjective = declensionAdjective;
function deriveComparison(root, adj, postfix, partOfSpeech) {
    const { isPositive, isComparative, isSuperlative } = parseComparisionModifiers(partOfSpeech);
...
function parseComparisionModifiers(partOfSpeech) {
...
const SYLLABIC_ENDING_REGEX = new RegExp(`([${substitutions_1.VOWELS}]st[ŕrl]|tv[rŕ]d|lst|č[rŕ]stv|m[rŕ]tv|brz)$`); // ostr, bystr, črstv, tvrd, mrtv, brz
const CONSONANTS_ENDING = new RegExp(`[j${substitutions_1.ALL_CONSONANTS}]*$`);
function short_form(adj, root, partOfSpeech) {
...
function comparative_adj(root) {
    ...
    else if (root.lastIndexOf('ok') == root.length - 2 ||
...
//# sourceMappingURL=declensionAdjective.js.map