...
describe('parsePos', () => {
  for (const [abbr, expected] of [
    ...
    test(`should expand ${JSON.stringify(abbr)} to object`, () => {
      const actual = parsePos(abbr);
      assert.ok(actual, 'should have returned non-null');
      ...
      assert.partialDeepStrictEqual(actual, pattern);
    });
  }

  it('should throw for non-strings', () => {
    assert.throws(() => parsePos(null as any), {
      message: 'Expected a string, but got: null',
    });
  });

  it('should throw for unknown abbreviations', () => {
    assert.throws(() => parsePos('unknown'), {
      message: 'Failed to parse the part of speech in: unknown',
    });
  });
});
