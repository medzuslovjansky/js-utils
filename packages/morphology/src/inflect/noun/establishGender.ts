export function establishGender(noun: string, gender: string): string {
  ...
  // `-a` goes to the a-stems whatever the dictionary says the gender is —
  // `sluga` and `tata` are masculine and decline like `žena`. `-i` does not:
  // the feminines in `-i` are `pani` and `bogyni`, while the masculines are
  // borrowings like `poni` that do not decline at all.
  if (lastChar === 'a' || (lastChar === 'i' && gender.charAt(0) !== 'm')) {
    return 'f1';
  }
  if (lastChar === 'ę') {
    return 'n2';
