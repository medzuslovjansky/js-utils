...
import { ALL_CONSONANTS, VOWELS } from '@interslavic/translit/phonology';

/**
 * One gender slot of a case. Three of them hold two forms rather than one, and
 * the difference is animacy: `dobrogo` is what an animate accusative takes and
 * `dobry` an inanimate one, `dobri` and `dobre` the same in the plural. Nothing
 * else in an adjective's paradigm has an alternative, so nothing else is
 * tagged.
 */
export type AdjectiveCell = Array<{ form: string; feats?: TokenFeatures }>;

export type AdjectiveParadigm = {
  singular: AdjectiveParadigm$Case;
  plural: AdjectiveParadigm$Case;
  short: string | undefined;
  comparison: Comparison;
};

type AdjectiveParadigm$Case = Record<keyof AdjectiveCases, AdjectiveCell[]>;
...
export function declineAdjective(
  ...
  return {
    singular: {
      ...
      acc: applyRules([m_acc_sg, n_nom_sg, f_acc_sg], postfix, [ANIMACY]),
    ...
    plural: {
      nom: applyRules([m_nom_pl, fn_nom_pl], postfix, [ANIMACY]),
      acc: applyRules([m_acc_pl, fn_nom_pl], postfix, [ANIMACY]),
...
function deriveComparison(
  ...
  return {
    positive: isPositive ? citations(applyRules([m_nom_sg, adv], postfix)) : [],
    comparative: isSuperlative
      ? []
      : citations(applyRules([comp_adj, comp_adv], postfix)),
    superlative: citations(applyRules([sup_adj, sup_adv], postfix)),
  };
}
...
/**
 * Comparison is the one place a cell turns back into a lemma: `derive` takes
 * these forms and builds whole paradigms out of them. Across the dictionary no
 * comparison slot ever holds more than one form, so one form is what comes
 * out — a second would be a word nobody could inflect, not a variant to print.
 */
function citations(slots: AdjectiveCell[]): string[] {
  return slots.map(([first]) => first.form);
}

function applyRules(
  arr: string[],
  postfix: string,
  pairs: ReadonlyArray<readonly [TokenFeatures, TokenFeatures]> = [],
): AdjectiveCell[] {
  return arr
    .map(rules)
    .map((item) =>
      item.replace(/$/, postfix).replace(/( *\/ *)/g, postfix + '$1'),
    )
    .map((item, slot) => {
      const forms = item.split('/').map((part) => part.trim());
      const pair = forms.length === 2 ? pairs[slot] : undefined;

      return forms.map((form, index) =>
        pair ? { form, feats: pair[index] } : { form },
      );
    });
}
...
export function inflectAdjective(
  ...
  for (const [upos, baseFeats] of variants) {
    if (upos !== 'ADJ' && upos !== 'VERB') continue;

    const mergedFeats = { ...baseFeats, ...itemFeats };
    const paradigm = declineAdjective(
      transliterate(word, 'isv-Latn-x-etymolog'),
      '',
      xpos,
    );

    const push = (slot: AdjectiveCell, feats: TokenFeatures) => {
      for (const form of slot) {
        results.push({
          form: form.form,
          upos,
          feats: { ...mergedFeats, ...feats, ...form.feats },
        });
      }
    };
    ...
    // The table lays the singular out by gender, but only the nominative and the
    // accusative keep all three apart; the oblique cases merge the masculine
    // with the neuter and print them once.
    for (const [caseName, slots] of Object.entries(paradigm.singular)) {
      const Case = CASES[caseName];

      const merged = caseName !== 'nom' && caseName !== 'acc';
      push(slots[0], { Number: 'Sing', Gender: 'Masc', Case });
    ...
    // The plural keeps a masculine apart only where animacy shows, and merges
    // the neuter with the feminine everywhere.
    for (const [caseName, slots] of Object.entries(paradigm.plural)) {
      const Case = CASES[caseName];

      if (caseName === 'nom' || caseName === 'acc') {
        push(slots[0], { Number: 'Plur', Gender: 'Masc', Case });
        push(slots[1], { Number: 'Plur', Gender: 'Neut', Case });
        push(slots[1], { Number: 'Plur', Gender: 'Fem', Case });
      } else {
        push(slots[0], { Number: 'Plur', Case });
      }
    }
  }
