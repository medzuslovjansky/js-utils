...
// Fed through `composePasses`, which merges whichever of these six prove
// mechanically safe to run as one table (see `fuse.ts`) -- comments above
// each still document the *original* ordering dependency, since that is
// what the soundness test is re-deriving, not guessing at.
const loosePasses = composePasses([
  IOTATED_CYRILLIC,
  SOFT_CONSONANTS,
  CYRILLIC_LETTERS,
  LATIN_CLUSTERS,
...
const looseReplacer = (word: string): string =>
  loosePasses.reduce((acc, pass) => pass(acc), word);
