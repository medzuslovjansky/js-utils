import { Glagolitic } from '../../../../packages/translit/src/constants/index.ts';

export const TransliterationType = {
  Latin: 1,
  ASCII: 2,
  Polish: 3,
  StandardCyrillic: 5,
...
export type TransliterationType =
  (typeof TransliterationType)[keyof typeof TransliterationType];

export const FlavorizationType = {
  CyrillicExtended: 1,
  Etymological: 2,
  Standard: 3,
  Slovianto: 4,
...
export type FlavorizationType =
  (typeof FlavorizationType)[keyof typeof FlavorizationType];
...
/**
 * Four phases, in order. Everything before the last one works on the
 * etymological alphabet plus a few private markers (`%` for a word
 * boundary, `#` for a delimiter, `ı` after a soft consonant); only
 * `toScript` leaves it.
 */
function transliterateWord(
  ...
  const resolved = resolveLiquids(__iW);
  const flavoured = flavorize(resolved, flav);
  return restoreCase(toScript(flavoured, type, flav), __OrigW, type);
}

/**
 * `r` and `ŕ` are syllabic between consonants and not otherwise, and the
 * spelling only says so indirectly. Resolving that first means every rule
 * after this one can read `ŕ`, `ř` and `ṙ` as three different letters.
 * The rest is spelling housekeeping: `x` to `ks`, the `ı` marker after a
 * soft consonant, the `#` delimiter before a `j` that is not a softener.
 */
...
/**
 * The etymological alphabet is the full one; a flavour is a projection of
 * it that throws some distinctions away — standard drops the nasals, the
 * northern one turns `ę` into `ja`. Nothing here changes script.
 */
export function flavorize(__iW: string, flav: string | number): string {
  let iW = __iW;
...
/** The one phase that leaves the etymological alphabet. */
...
export function jgedoe(iW: string) {
  /* (V)j(V)	= j */
  /* Cj(C)  	= Ь */
  /* CjV		= ь */
...
export function nmsifyLoose(iW: string) {
  return (
    iW
      .replace(/[яꙗ]/g, '#a')
      .replace(/ьа/g, '#a')
...
export function nmsifyStrict(iW: string) {
  return iW
