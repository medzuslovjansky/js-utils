 ...
 */
const resolveNadsException = createReplacer({
  nadse: 'nace',
  nadsę: 'nacę',
});
 ...
 */
const markGlideAfterSoftConsonant = createReplacer({
  ľıj: 'ľi̯',
  ńıj: 'ńi̯',
  řıj: 'ři̯',
  ťıj: 'ťi̯',
 ...
 */
const vowelsAndSyllabicLiquids = createReplacer({
  e: 'ɛ',
  ė: 'ɜ',
  ě: 'ьɛ',
  ę: 'ьæ',
 ...
 */
const dropSoftMarkerAfterSibilant = createReplacer({
  šь: 'š',
  čь: 'č',
  žь: 'ž',
  jь: 'j',
 ...
 * output either.
 */
const affricatesAndFricatives = createReplacer({
  c: 't͡s',
  č: 't͡ʃ',
  šć: 'ɕt͡ɕ',
  ć: 't͡ɕ',
 ...
 */
const palatalize = createReplacer({
  sť: 'sʲtʲ',
  zď: 'zʲdʲ',
  ť: 'tʲ',
  tь: 'tʲ',
 ...
 * as a string.
 */
const revertDarkLBeforeFrontVowel = createReplacer({
  ɫi: 'li',
  ɫ̯: 'l̯',
  ɫe: 'le',
});
 ...
 */
const palatalizeAffricateBeforeGlide = createReplacer({
  t͡sj: 't͡sʲ',
  d͡zj: 'd͡zʲ',
});
...
const finalCleanup = createReplacer({
  jj: 'j',
  '#': '',
  ı: '',
  '%': '',
...
export function latn2ipa(word: string, flavour: Flavour): string {
  void flavour;
  const transformed = PASSES.reduce((acc, pass) => pass(acc), word);
  return finalCleanup(transformed);
}
